...
class ChatWithCoachVC: UIViewController{
    //Custom Navbar
    ...
    var allchatData = AllUserChatModel()
...
//    let chatArray = ["hello", "hello how are you", "My wife was great", "hjs hjsggvdfv hhg igig iegeif giegheig eiheiuh iuvihei huieh ehueuhehfuashf uoiehuegh uehg ueh oeh regheo", "hbvdbfvbdffv mvbjkdfbv dfmvbdfjb v vjf v", "hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh", "jkdbjkdfbjkbjv jd hvuhohd oihdf oihohvo iaioaji oufiref peiuioeuie vdhvduihsu soihia osvioviof viojvioej udhdhhdfh df hodh odvoih  diojaioj iodj iojv iodaj ioajioioa eguio aerioepj ioe "]
    ...
    override func viewDidLoad() {
        ...
        setupUI()
        setupTableView()
        chatViewModel.Chatdelegate = self
        allchatData.AllUserChatdelegate = self
        getChatDetails()
...
//        self.showAlert(title: ConstantAlertTitle.LuvoAlertTitle, message: "Under development")
    ...
    @IBAction func btnSendAction(_ sender: Any) {
        ...
        if (connectionStatus == false) {
            ...
            self.view.endEditing(true)
            self.view.startActivityIndicator(title: ConstantActivityIndicatorMessage.pkLoading, color: .white)
            let request = UserChatRequest(text: messageTV.text!, sessionId: sessionId)
          ...
          //  var str : String? = sessionId
            ...
            self.ref.child(sessionId).setValue(["value": "1672237568, Type :Chat"])
...
extension ChatWithCoachVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if data?.senderID == userId {
            if data?.message == nil {
                let cell = chatTableView.dequeueReusableCell(withIdentifier: "UserChatReceivedImageTableViewCell") as! UserChatReceivedImageTableViewCell
                cell.selectionStyle = .none
                cell.UserupdtaeReceivedImageTableCellData(cellData: data!)
                ...
                let cell = chatTableView.dequeueReusableCell(withIdentifier: "UserChatReceiveTableViewCell") as! UserChatReceiveTableViewCell
                cell.selectionStyle = .none
               // cell.UserupdtaeReceivedTableCellData(cellData: data!)
...
extension ChatWithCoachVC: UserchatWithCoachModelDelegate {
    func didReceiveUserchatWithCoachResponse(ChatResponse: UserchatWithCoachResponse?) {
        ...
        if(ChatResponse?.status != nil && ChatResponse?.status?.lowercased() == ConstantStatusAPI.success)
        {
            ...
            allchatData.getChatDetails(token: token, SessionID: ChatResponse?.results?.sessionId ?? "test")
...
extension ChatWithCoachVC : AllUserChatModelDelegate
{
    func didReceiveAllUserChatResponse(ChatAllResponse: AllUserChatResponse?) {
        ...
        {
            debugPrint(ChatAllResponse?.results?.count ?? "test")
            
            ref.child("63ac266d23321847dc86fc08/\("value")").getData(completion:  { error, snapshot in
...
//            if let arrMessage = ChatAllResponse?.results {
//             // let reverseArray: [Message] = arrMessage
//             // let arrayWithSection = chatViewModel.getChatWithSection(chatArray: reverseArray)
//                messageArraySection = ChatAllResponse?.results
//            }
        }
